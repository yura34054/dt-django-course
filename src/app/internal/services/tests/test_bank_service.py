...
def test_send_money_account(setup_bank_account):
    ...
    assert (
        bank_service.send_money_account(owner_id, receiver_username, account_name, receiver_account_name, 100)
    ...
    assert (
        bank_service.send_money_account(owner_id, receiver_username, "abc", receiver_account_name, 100)
        == 'Account "abc" not found'
    )
    ...
    assert (
        bank_service.send_money_account(owner_id, receiver_username, account_name, "abc", 100)
        == 'Receiver account "abc" not found'
    )
...
def test_send_money_card(setup_bank_account):
    ...
    assert (
        bank_service.send_money_card(owner_id=owner_id, card_id=0, receiver_card_id=0, amount=100)
        == 'Card "0" not found'
    )
    ...
    assert (
        bank_service.send_money_card(
            owner_id=owner_id, card_id=receiver_card_id, receiver_card_id=sender_card_id, amount=100
        )
        == f'You don\'t own card "{receiver_card_id}"'
    )

    assert (
        bank_service.send_money_card(owner_id=owner_id, card_id=sender_card_id, receiver_card_id=0, amount=100)
        == 'Receiver card "0" not found'
    )

    assert (
        bank_service.send_money_card(
            owner_id=owner_id, card_id=sender_card_id, receiver_card_id=receiver_card_id, amount=100
        )
